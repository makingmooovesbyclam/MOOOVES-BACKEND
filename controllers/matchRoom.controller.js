...
exports.createMatchRoom = async (req, res) => {
  ...
  try {
    const matchCode = crypto.randomBytes(3).toString('hex').toUpperCase(); // 6-char code
    const room = new MatchRoom({
      user: userId,
      gameType,
      inviteCode: matchCode,
      status: 'waiting'
    });
...
exports.joinMatchByCode = async (req, res) => {
  const { matchCode, userId } = req.body;

  try {
    const room = await MatchRoom.findOne({ inviteCode: matchCode });
    if (!room) return res.status(404).json({ error: 'Invalid code' });

    if (!room.player1) room.player1 = userId;
...
exports.getAllMatchRooms = async (req, res) => {
   ...
   } catch (err) {     res.status(500).json({ error: 'Could not fetch rooms', details: err.message });
   }
 };
